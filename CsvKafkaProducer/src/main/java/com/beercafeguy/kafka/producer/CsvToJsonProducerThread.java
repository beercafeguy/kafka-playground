...
public class CsvToJsonProducerThread extends Thread{
    private final KafkaProducer<Integer, JsonNode> kafkaProducer;
    ...
    public CsvToJsonProducerThread(String topicName) {
    ...
    public void run() {
        try (Reader in = new FileReader("src/main/resources/employee.csv")) {
            ...
            for (CSVRecord csvRecord : csvRecords) {
                ...
                Employee employee=new Employee(Integer.parseInt(csvRecord.get("id")),csvRecord.get("name"),Integer.parseInt(csvRecord.get("salary")));
                ...
                if (isAsync) {
                    ...
                    ProducerRecord<Integer, JsonNode> record = new ProducerRecord<Integer, JsonNode>(topicName, key, jsonNode);
                    try {
                        kafkaProducer.send(record).get();
                        System.out.println("Sent message: (" + key + ", " + employee + ")");
                    ...
                    ProducerRecord<Integer, JsonNode> record = new ProducerRecord<Integer, JsonNode>(topicName, key, jsonNode);
                    kafkaProducer.send(record, new MyCallbackJson(key, employee, startTime));
                    System.out.println("Sent message: (" + key + ", " + employee + ")");
